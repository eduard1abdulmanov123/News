...
object Paginator {
    ...
    sealed class State {
        ...
        data class RefreshAfterEmptyError(val error: Throwable) : State()
        ...
        return when (action) {
            ...
            is Action.NewPage<*> -> {
                ...
                when (state) {
                    ...
                    is State.RefreshAfterEmptyError -> State.Data(1, items)
            ...
            is Action.PageError -> {
                when (state) {
                    ...
                    is State.RefreshAfterEmptyError -> State.EmptyError(action.error)
                    is State.Refresh<*> -> {
